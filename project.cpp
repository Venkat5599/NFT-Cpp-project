...
class Discount {
public:
    virtual double calculate(NFT cart[], int n, double total, bool member, string bank, string coupon) = 0;
};
...
public:
    double calculate(NFT cart[], int n, double total, bool member, string bank, string coupon) {
        double sum = 0;
        for(int i=0;i<n;i++)
...
// POLYMORPHISM: CouponDiscount overrides the virtual calculate method
// Provides 20% discount when coupon code "nft20" is entered (case-insensitive)
class CouponDiscount : public Discount {
private:
...
public:
    double calculate(NFT cart[], int n, double total, bool member, string bank, string coupon) {
        string cleanCoupon = removeQuotes(toLower(coupon));
        if(cleanCoupon == "nft20")
            return total * 20 / 100; // 20% discount with nft20 coupon
        return 0;
    }
};
...
int main() {
    ...
    for(int i=0;i<5;i++)
        totalDiscount += discounts[i]->calculate(cart.getCart(), cart.getCount(), total, loyalty, bank, coupon);
